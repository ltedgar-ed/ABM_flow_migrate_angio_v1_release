...
for t = num_timesteps:num_timesteps
close (1)
    ...
    if (x_range > y_range)
        ...
        set(gcf, 'Position', [50 50 500 (y_range/x_range)*500])
    end
    ...
    text(min(nodes(:,1))-30, min(nodes(:,2))-8, ['  P_i_n = ' num2str(round(nodal_pressures(PBCs(1,1), t))) ' Pa'],'Color','k','FontSize', pf)
    text(max(nodes(:,1))-30, min(nodes(:,2))-8, ['P_o_u_t = ' num2str(round(nodal_pressures(PBCs(2,1), t))) ' Pa'],'Color','k','FontSize', pf)
