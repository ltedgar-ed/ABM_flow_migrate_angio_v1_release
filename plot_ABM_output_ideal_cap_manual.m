%function [] = plot_ABM_output_ideal_cap(file_name)
...
close (1)
    ...
    set(gcf, 'Position', [50 50 (x_range/y_range)*500 500])
    ...
    for v = 1:num_vess
        ...
        if (vess_diameter(v,t) ~= 0)
            plot([x1 x2], [y1 y2], 'Color', vess_colors(v,:), 'MarkerSize',14,'LineWidth', vess_diameter(v, t)/2)
        end
    ...
    title(['Time Step ' num2str(t-1)])
    
    text(min(nodes(:,1))-30, min(nodes(:,2))-8, ['  P_i_n_1 = ' num2str(round(nodal_pressures(PBCs(1,1), t))) ' Pa'],'Color','k','FontSize', pf)
    text(min(nodes(:,1))-30, max(nodes(:,2))+8, ['P_o_u_t_1 = ' num2str(round(nodal_pressures(PBCs(2,1), t))) ' Pa'],'Color','k','FontSize', pf)
    text(max(nodes(:,1))-30, min(nodes(:,2))-8, ['  P_i_n_2 = ' num2str(round(nodal_pressures(PBCs(3,1), t))) ' Pa'],'Color','k','FontSize', pf)
    text(max(nodes(:,1))-30, max(nodes(:,2))+8, ['P_o_u_t_2 = ' num2str(round(nodal_pressures(PBCs(4,1), t))) ' Pa'],'Color','k','FontSize', pf)
